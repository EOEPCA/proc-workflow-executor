...
def prepare(state, namespace, volume_size, volume_name):
    ...
    resp_status = prepare.run(
        namespace=namespace, volumeSize=volume_size, volumeName=volume_name, state=state
    )
...
def status(state, namespace, workflow_name):
    ...
    resp = status.run(namespace, workflow_name, state)
    return resp
...
cli = click.CommandCollection(
    sources=[prepare_cli, execute_cli, status_cli, result_cli]
)
