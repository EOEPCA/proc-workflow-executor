...
class DemoClient:
    ...
    def get_token_endpoint(self):
        ...
        if self.token_endpoint == None:
            ...
            r = self.http_request(
                "GET",
                self.base_url + "/.well-known/uma2-configuration",
                headers=headers,
            )
    ...
    def get_id_token(self, username, password):
        ...
        r = self.http_request(
            "POST", self.get_token_endpoint(), headers=headers, data=data
        )
    ...
    def register_protected_resource(
        self, resource_api_url, uri, id_token, name, scopes
    ):
        ...
        if resource_id == None:
            ...
            r = self.http_request(
                "POST", f"{resource_api_url}/resources", headers=headers, json=data
            )
    ...
    def get_access_token_from_ticket(self, ticket, id_token):
        ...
        print(
            f"Calling token endpoint with ID Token + ticket: POST => {token_endpoint}"
        )
            ...
            # if we got a 401 then initiate the UMA flow
                ...
                if id_token is not None:
                    ...
                    if ticket is not None:
                        print(
                            "Got ticket from response. Using ID Token + ticket to request an RPT"
                        )
                        access_token = self.get_access_token_from_ticket(
                            ticket, id_token
                        )
                        repeat = True
                else:
    ...
    def wps_get_capabilities(self, service_base_url, id_token=None, access_token=None):
        ...
        r, access_token = self.uma_http_request(
            "GET", url, id_token=id_token, access_token=access_token
        )
    ...
    def proc_list_processes(self, service_base_url, id_token=None, access_token=None):
        ...
        r, access_token = self.uma_http_request(
            "GET", url, headers=headers, id_token=id_token, access_token=access_token
        )
    ...
    def proc_deploy_application(
        self,
        service_base_url,
        app_deploy_body_filename,
        id_token=None,
        access_token=None,
    ):
        ...
        try:
            ...
            print(
                f"ERROR could not find application details file: {app_deploy_body_filename}"
            )
        except json.decoder.JSONDecodeError:
            print(
                f"ERROR loading application details from file: {app_deploy_body_filename}"
            )
        # make request
        url = service_base_url + "/processes"
        ...
        r, access_token = self.uma_http_request(
            ...
            json=app_deploy_body,
        )
        print(f"[Deploy Response] = {r.status_code} ({r.reason})")
        return r, access_token

    @keyword(name="Proc App Details")
    def proc_get_app_details(
        self, service_base_url, app_name, id_token=None, access_token=None
    ):
    ...
    def proc_execute_application(
        self,
        service_base_url,
        app_name,
        app_execute_body_filename,
        id_token=None,
        access_token=None,
    ):
        ...
        try:
            with open(app_execute_body_filename) as app_execute_body_file:
                ...
                print(
                    f"Application execute details read from file: {app_execute_body_filename}"
                )
        except FileNotFoundError:
            print(
                f"ERROR could not find application execute details file: {app_execute_body_filename}"
            )
        except json.decoder.JSONDecodeError:
            print(
                f"ERROR loading application execute details from file: {app_execute_body_filename}"
            )
        # make request
        ...
        try:
            ...
            job_location = None
        print(
            f"[Execute Response] = {r.status_code} ({r.reason}) => job={job_location}"
        )
        return r, access_token, job_location

    @keyword(name="Proc Job Status")
    def proc_get_job_status(
        self, service_base_url, job_location, id_token=None, access_token=None
    ):
    ...
    def proc_poll_job_completion(
        ...
        while status == "running":
            r, access_token, status = self.proc_get_job_status(
                service_base_url,
                job_location,
                id_token=id_token,
                access_token=access_token,
    ...
    def proc_undeploy_application(
        ...
        headers = {"Accept": "application/json"}
        r, access_token = self.uma_http_request(
            "DELETE", url, headers=headers, id_token=id_token, access_token=access_token
        )
    ...
    def update_policy(
        self, pdp_base_url, policy_cfg, resource_id, id_token=None, policy_id=None
    ):
        """Updates a policy
    ...
    def clean_state_resources(self, pep_resource_url, id_token):
        ...
        for k in self.state["resources"][pep_resource_url]:
            res = self.http_request(
                ...
                pep_resource_url
                + "/resources/"
                + str(self.state["resources"][pep_resource_url][k]),
                headers=headers,
                verify=False,
            )
    ...
    def clean_owner_resources(self, pep_resource_url, id_token):
        ...
        res = self.http_request(
            "GET", pep_resource_url + "/resources", headers=headers, verify=False
        )
        for k in json.loads(res.text):
            res = self.http_request(
                "DELETE",
                pep_resource_url + "/resources/" + k["_id"],
                headers=headers,
                verify=False,
            )

    @keyword(name="Workspace Get Details")
    def workspace_get_details(
        self, service_base_url, workspace_name, id_token=None, access_token=None
    ):
    ...
    def reset_resource_policy(
        self, resources_endpoint, pdp_endpoint, id_token, resource_uri
    ):
        ...
        resource_id = self.get_resource_by_uri(
            resources_endpoint, resource_uri, id_token
        )
        policy = {
            "name": "Reset Policy",
